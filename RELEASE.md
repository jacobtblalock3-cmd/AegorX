...
2. Bump `version` in `pyproject.toml` and `defentra/__init__.py`; commit.
3. Tag and push:
...
4. The **Release** workflow builds automatically:
   ...
   - `defentra_<version>_all.deb` (Debian/Ubuntu clients)
...
| Channel | Command |
|---|---|
| Debian / Ubuntu | `sudo apt install ./defentra_<version>_all.deb` then enable `defentra-monitor.service` |
| pip (any Linux) | `pip install defentra` |
| Source | `pip install git+https://github.com/jacobtblalock3-cmd/defentra.git@v<version>` |

After install:

```bash
sudo systemctl enable --now defentra-monitor     # realtime protection (fanotify)
sudo systemctl enable --now defentra-feed-update.timer  # daily signed intel
defentra model fetch                              # ML detection out-of-the-box
```
...
|---|---|---|
| `SIGNING_PRIVATE_KEY` | Signs feeds + model metadata | `~/.defentra-signing/signing_private.pem` on the release host |
