...
automatically, and prove whether its trust anchors were touched:
  ...
  * Heartbeat               - periodic liveness record consumed by
    `aegorx watchdog`, which restarts the service when it goes stale.
...
class Heartbeat:
    ...
    def start(self) -> None:
        self._write()
        self._thread = threading.Thread(target=self._loop, daemon=True, name="aegorx-heartbeat")
        self._thread.start()
...
def liveness(max_age_seconds: float = 90.0) -> Dict:
    ...
    try:
        ...
        return {"healthy": False, "reason": "corrupt heartbeat"}
    alive = _pid_alive(pid)
    healthy = age <= max_age_seconds and alive
    return {"healthy": healthy, "pid": pid, "age_seconds": round(age, 1), "process_alive": alive}
...
def restart_service(unit: str = "aegorx-monitor") -> bool:
    """Best-effort systemd restart; returns True when the restart was issued."""
    import subprocess

    _ALLOWED_UNITS = frozenset({"aegorx-monitor", "aegorx-agent"})
...
def _candidate_targets() -> List[str]:
    ...
    from aegorx.signing.keys import PACKAGE_TRUST_DIR
