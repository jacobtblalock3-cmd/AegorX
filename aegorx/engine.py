...
from aegorx.ml.features import extract_features, looks_executable, vectorize
from aegorx.scanner import archives, office, pdfdoc
from aegorx.scanner.archives import ArchiveLimits
from aegorx.scanner.hashes import FAST_BACKEND, file_hashes
...
class ScanEngine:
    ...
    def __init__(
        ...
        from aegorx.rules_store import rules_dir as state_rules_dir
    ...
    def scan_file_descriptor(self, fd: int, path_hint: str = "") -> FileScanResult:
        ...
        from aegorx.scanner.hashes import file_hashes_fd
        ...
        try:
            ...
            with os.fdopen(dup, "rb") as fh:
                data = fh.read(self.max_file_size + 1)
            dup = -1  # fd now owned by fdopen context manager
                ...
                if not office.OFFICE_AVAILABLE and not detections:
                    detections.append(
                        Detection(
                            ...
                            severity=3,
                            details={"hint": "pip install 'aegorx[office]'"},
                        )
                    )
            else:
    ...
    def _scan_archive_contents(
        ...
        """
        workdir = tempfile.mkdtemp(prefix="aegorx-arch-")
        try:
            try:
