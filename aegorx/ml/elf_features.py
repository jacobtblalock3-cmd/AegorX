...
def _parse_elf_inner(data: bytes) -> Dict:
    ...
    # Bounds validation for header offsets
    if e_phoff > len(data) or e_shoff > len(data):
        raise NotElfError("ELF header offsets exceed file size")
    ...
    for i in range(shnum):
        ...
        if sh_type == SHT_PROGBITS:
            # Validate sh_offset and sh_size against file bounds
            if sh_offset < len(data) and sh_offset + sh_size <= len(data):
