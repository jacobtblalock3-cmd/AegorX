...
def select_backend(name: str, paths: List[str], excludes: Optional[List[str]] = None) -> BackendBase:
    ...
    if name == "es":
        from aegorx.realtime.es_backend import create_es_backend

        return create_es_backend(paths, excludes=excludes)
    if name == "minifilter":
        from aegorx.realtime.minifilter_backend import create_minifilter_backend
    ...
    if system == "Darwin":
        ...
        from aegorx.realtime.es_backend import EndpointSecurityBackend
        ...
        if EndpointSecurityBackend.available():
            try:
                ...
                pass
        from aegorx.realtime.fswatch_backend import FSwatchMacOSBackend
    ...
    if system == "Windows":
        ...
        from aegorx.realtime.minifilter_backend import MinifilterBackend
        ...
        if MinifilterBackend.available():
            try:
                ...
                pass
        from aegorx.realtime.fswatch_backend import FSwatchWindowsBackend
...
class RealTimeMonitor:
    ...
    def __init__(
        ...
        self.pool = ThreadPoolExecutor(max_workers=max(1, workers), thread_name_prefix="aegorx-scan")
    ...
    def run(self) -> None:
        ...
        from aegorx.shield import Heartbeat, install_signal_handlers
