...
`aegorx doctor` probes every protection layer read-only and prints an
...
def _check_signatures() -> Dict:
    ...
    if count <= 0:
        return _result(STATUS_WARN, "signature database empty — run 'aegorx db seed' or 'feed update'")
...
def _check_model() -> Dict:
    ...
    return _result(
        STATUS_WARN,
        "no ML model — run 'aegorx model fetch' (detection relies on signatures/YARA)",
    )
...
def _check_feed_freshness() -> Dict:
    ...
    try:
        with open(state_path, "r", encoding="utf-8") as fh:
            data = json.load(fh)
        raw = data.get("last_generated_utc", "") if isinstance(data, dict) else ""
    except (OSError, json.JSONDecodeError):
        return _result(STATUS_WARN, "no feed ever applied — run 'aegorx feed update'")
    if not raw:
        return _result(STATUS_WARN, "no feed ever applied — run 'aegorx feed update'")
...
def _check_trust_keys() -> Dict:
    from aegorx.signing.keys import trusted_key_paths
...
def _check_agent_pairing() -> Dict:
    from aegorx.management.agent import AgentConfigError, load_config
