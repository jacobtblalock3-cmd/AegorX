...
"""PyInstaller spec for standalone Defentra builds (Windows / macOS / Linux).
...
trust_keys = glob.glob(os.path.join(ROOT, "defentra", "signing", "trusted_keys", "*.pub"))
rule_files = glob.glob(os.path.join(ROOT, "rules", "*.yar"))

datas = [(p, "defentra/signing/trusted_keys") for p in trust_keys]
