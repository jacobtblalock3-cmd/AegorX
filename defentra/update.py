...
from defentra.signing.feed import canonical_payload, check_expiry
from defentra.signing.keys import public_key_fingerprint, trusted_key_paths

UPDATE_FORMAT = "defentra-update-manifest"
MANIFEST_VERSION = 1
...
def fetch_manifest(url: Optional[str] = None, opener=None) -> Dict:
    from defentra.signing.feed import FeedError, fetch_feed
...
def verify_manifest(doc: Dict, extra_trusted_keys: Optional[List[str]] = None) -> str:
    ...
    if not isinstance(doc, dict) or doc.get("format") != UPDATE_FORMAT:
        raise UpdateError("not a defentra update manifest (bad 'format')")
    ...
    for path in candidates:
        try:
            from defentra.signing.keys import load_public_key
...
def check(
    ...
    from defentra.signing.feed import FeedError
...
def auto_apply(
    ...
    dest = dest_dir or tempfile.mkdtemp(prefix="defentra-update-")
    try:
