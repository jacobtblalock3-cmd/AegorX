...
def status_snapshot(state_dir_path: Optional[str] = None) -> Dict:
    ...
    from defentra.shield import liveness
    ...
    feed_rules = 0
    try:
        from defentra.rules_store import current_rules
...
def run(stdscr) -> None:
    ...
    while True:
        ...
        if key == ord("q"):
            ...
            dash.run_action_async("rules", lambda: f"active rule sets: {len(dash.snapshot.get('feed_rules', []) )}" )
        elif key == ord("p"):
            from defentra import shield
